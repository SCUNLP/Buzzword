...
def apply_mask(words, examples):
    pos = []
    for idx, (word, example) in enumerate(zip(words, examples)):
        length = len(word) - 2
        for i in range(len(example)):
            if not (example[i:i+length] - word[1:1+length]).any():
                example[i:i+length] = torch.full(np.shape(word[1:1+length]), mask)
...
class BertSelecting():
    def __init__(self):
        self.tokenizer = BertTokenizer.from_pretrained('../Chinese_BERT_wwm/')
        self.model = BertForMaskedLM.from_pretrained('../Chinese_BERT_wwm/')
    ...
    def decode(self, logits, words, pos):
        ...
        for i in range(len(pos)):
            ...
            for j in range(1, len(words[i]) - 1):
                prob.append(pred[i][pos[i]+j-1][words[i][j]])
            prob = np.array(prob)
...
if __name__ == '__main__':
    ...
    with open('res_parallel.json', 'r', encoding='utf-8') as f:
        result = json.load(f)
    ...
    for items in dataloader:
        ...
        with torch.no_grad():
           logits, words_tokens, pos = model.encode(words, examples)
           ...
           print(res)
           with open('res_parallel.json', 'w', encoding='utf-8') as f:
